...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `workflow_run` | Execute a multi-agent workflow script. Workflows decompose large tasks into independent phases with parallel agents and structured data flow. Use for tasks with 3+ independent subtasks (audits, research, migrations) feeding into a synthesis step. The workflow script is a Python file defining a `workflow()` function using `phase()`, `run()`, and `run_all()` primitives. Supports checkpoint resume and force re-run. | — |
| `workflow_list` | List available workflow scripts. Returns workflow names, paths, and descriptions from `workflows/` and `~/.hermes/workflows/` directories. | — |
